...
function insertar_registro_evaluacion($cod_proyecto) {
    $con = new Conexion();
    $c = $con->getConection();
    //cambiar los valores de usuario y codigo de docente
    pg_query($c, "INSERT INTO registro_evaluacion_final(consultor_usuario_idusuario, consultor_idconsultor, proyecto_codproyecto) VALUES (2, 1, '$cod_proyecto');");
    pg_close($c);
}
...
function mostrar_lista_criterios($cod_consultor, $cod_ge) {
    $con = new Conexion();
    $c = $con->getConection();
    ...
    while ($f = pg_fetch_object($consulta)) {
        ...
        echo "<tr>"
        . "<td><a href='../Vista/iuEvaluacionFinalDocenteGE.php?a=$cod_ge&cc=$id_crit&nc=$nombre&pc=$porcen_calif&tc=$tipo&it=$id_tipo'>$nombre</td><td>$tipo</td><td>$porcen_calif</td>"
        . "</tr>";
    }
    exit();
    pg_close($c);
...
function registrar_evaluacion_final($cod_ge, $id_criterio, $id_tipo_criterio, $nombre_concepto, $observaciones) {
    $con = new Conexion();
    $c = $con->getConection();

    $cons_id_usuario_ge = pg_query($c, "select usuario_idusuario from grupo_empresa where codgrupo_empresa = " . $cod_ge . ";");
    $id_usuario_conseguido = pg_fetch_object($cons_id_usuario_ge);
    $usr_ge = $id_usuario_conseguido->usuario_idusuario;
    ...
    $id_cons = $datos_criterio_conseguidos->registro_evaluacion_final_consultor_idconsultor;
    $usr_cons = $datos_criterio_conseguidos->registro_evaluacion_final_consultor_usuario_idusuario;
    ...
    if (no_existe_registro_EF($cod_ge, $id_det_concepto_criterio)) {
...
function no_existe_registro_EF($cod_ge, $id_det_concepto_criterio) {
    $con = new Conexion();
    $c = $con->getConection();
    $consulta_cantidad_conseguida = pg_query($c, "select count(*) from evaluacion_final where grupo_empresa_codgrupo_empresa = $cod_ge and detalle_criterio_iddetalle_criterio = $id_det_concepto_criterio;");
...
function registrar_evaluacion_final_numerico($cod_ge, $id_criterio, $id_tipo_criterio, $observaciones, $nota) {
    $con = new Conexion();
    $c = $con->getConection();
